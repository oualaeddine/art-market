...
class YalidineMairiesArab extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $collection_m = new Collection();
        foreach($json_mairie as $item){
                    $collection_m->push((object)$item);
                }

        $yaldine_mairie = YalidineMairie::get();

        foreach($yaldine_mairie as $y){

            $name  = $collection_m->where('name',$y->name)->first();
            if($name){

                $y->name_ar = $name->ar_name;
                $y->save();
