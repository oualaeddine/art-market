...
class ShowArchivedCategories
{
    ...
    public function asController(Request $request)
    {
        ...
        $breadcrumbs = array(['name' => "Catégories", 'url' => route('admin.categories.index')],['name' => "Catégories archivés", 'url' => route('admin.categories.archived')]);

        if ($request->ajax()) {
            $data =  Category::query()->onlyTrashed()->orderby('created_at', 'desc');
            return DataTables::of($data)
                    ->addIndexColumn()
                    ->addColumn('action','CategoriesUi::actions.btn-archived')
        ...
        return view('CategoriesUi::index-archived', compact('header', 'user_info','breadcrumbs'))->with(['page_title' => "Catégories archivés"]);
    }
