...
class StoreOrderVendor
{
    ...
    public function asController(ActionRequest $request,Vendor $vendor)
    {
        ...
        try{
           ...
           $order= $this->handle($request,$vendor);
    ...
    public function handle(ActionRequest $request,$vendor)
    {
        ...
        while (Order::query()->where('tracking_code', $code)->exists()|| RawOrder::query()->where('tracking_code', $code)->exists()){
            ...
            $order = Order::create($this->getOrderFields($request)+[  'updated_by'=>auth()->id(),
                    'tracking_code'=>$code,'address_id'=>$address->id,'client'=>$request->client_id,'vendor_id'=>$vendor->id]);
