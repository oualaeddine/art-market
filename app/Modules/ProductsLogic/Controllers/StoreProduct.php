...
class StoreProduct
{
    ...
    public function handle(ActionRequest $request,$vendor): Product
    {
        ...
        if ($request->image != null) {
            ...
            return  $product = Product::create($this->getProductFields($request) + ['is_active'=>true,'vendor_id'=>$vendor->id,'image' => $path , 'rating' => $random_rat]);
        } else {
            return  $product = Product::create($this->getProductFields($request) + ['is_active'=>true,'vendor_id'=>$vendor->id,'rating' => $random_rat]);
        }
